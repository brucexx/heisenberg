...
public class BlockingSession implements Session {
	...
	private XAMultiNodeExecutor xaMNode;
	...
	public BlockingSession(ServerConnection source) {
		...
		xaMNode = new XAMultiNodeExecutor();
...
//			xaSNode.execute(rrs.getNodes()[0], this, rrs.getFlag(), oSql, 0);
//		} else {
			xaMNode.setSql(oSql);
			xaMNode.execute(nodes, this, rrs.getFlag(), type, 0);
//		}
	}
	 ...
	 * @param type
	 */
	public void endAndPrepare(RouteResultset rrs, String xId, int type) {
		// 检查路由结果是否为空
		RouteResultsetNode[] nodes = rrs.getNodes();
		if (nodes == null || nodes.length == 0) {
			source.writeErrMessage(ErrorCode.ER_NO_DB_ERROR, "No dataNode selected");
...
//		if (nodes.length == 1 && nodes[0].getSqlCount() == 1) {
//			xaSNode.execute(rrs.getNodes()[0], this, rrs.getFlag(), "commit_sql_mock", 1);
//		} else {
			xaMNode.execute(nodes, this, rrs.getFlag(), type, 1);
//		}
	}

	public void rollback(RouteResultset rrs, String xId) {
		// 检查路由结果是否为空
		RouteResultsetNode[] nodes = rrs.getNodes();
		if (nodes == null || nodes.length == 0) {
			source.writeErrMessage(ErrorCode.ER_NO_DB_ERROR, "No dataNode selected");
...
//		if (nodes.length == 1 && nodes[0].getSqlCount() == 1) {
//			xaSNode.execute(rrs.getNodes()[0], this, rrs.getFlag(), "rollback_mock", 3);
//		} else {
			xaMNode.execute(nodes, this, rrs.getFlag(), ServerParse.ROLLBACK, 3);
//		}
	}
