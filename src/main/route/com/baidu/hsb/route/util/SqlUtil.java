...
public class SqlUtil {
		...
		if (prefixSet != null && !prefixSet.isEmpty() && StringUtil.isNotEmpty(oSql)) {
			...
			if (ast instanceof DMLStatement) {
				try {
					...
					throw new RuntimeException("oSql parse error");
				}
	...
	private static void renderInsert(List<String> sqlStrs, SQLStatement ast, SQLStatement newAst, SchemaConfig schema,
		...
		// db index和val值
		Map<Integer, Map<String, List<Object>>> idxValMap = VelocityUtil.evalDBRuleMap(tc.getRule(), columnValues);

		Map<String, Set<Object>> tbPrefixSet = VelocityUtil.evalTbRuleArrayWithResult(tc.getRule(),
				idxValMap.get(dnIdx));
